...
async function syncProgress() {
  ...
  if (!u) return false;
  if (!POST_URL) {
    saveSyncStatus("local", "서버 저장 주소가 없어 이 기기에만 기록됩니다.");
    return false;
  }
  ...
  try {
    ...
    if (!data.ok || !data.progress) {
      saveSyncStatus("error", "서버 기록을 확인하지 못했습니다. 기록은 이 기기에 저장되어 있습니다.");
      return false;
    }
    ...
    fetch(POST_URL, {
      ...
      .then((data) => {
        if (data && data.ok) {
          ...
          saveSyncStatus("error", "서버 저장 응답을 확인하지 못했습니다. 기록은 이 기기에 저장되어 있습니다.");
        }
      })
      .catch(() => {
...
function postChallenge(verse, mode) {
  ...
  if (!u) return;
  if (!POST_URL) {
    saveSyncStatus("local", "서버 저장 주소가 없어 도전 기록은 이 기기에만 남습니다.");
    return;
  }
    ...
    fetch(POST_URL, {
      ...
      .then((data) => {
        if (data && data.ok) {
          ...
          saveSyncStatus("error", "도전 기록 저장 응답을 확인하지 못했습니다. 기록은 이 기기에 저장되어 있습니다.");
        }
      })
      .catch(() => {
